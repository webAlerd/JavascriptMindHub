// array con eventos pasados
function arrayPast(array){
    let arrayPastFilter = array.filter(past => past.date < data.currentDate)
    return arrayPastFilter
}
arrayPast(data.events)
// 
...
function filter() {
    let primerFiltro = filtrarSearch(arrayPast(data.events), search.value)
...
crearCards(arrayPast(data.events))
crearCheckbox(arrayPast(data.events))
